...
class MultimodalCrossAttentionClassifier(nn.Module):
    def __init__(self, num_classes, d_model=256, hidden_dim=128, dropout=0.5):
        ...
        self.raman_extractor = VGG(depth=11)  # Replaced AlexNet with VGG
        self.text_model = AutoModel.from_pretrained("distilbert-base-uncased")
        self.text_proj = nn.Linear(self.text_model.config.hidden_size, d_model)
        self.raman_proj = nn.Linear(512, d_model)  # Project VGG output (512) to d_model
...
# 计算测试集的指标
# 确保 test_labels 和 test_preds 已填充
...
def compute_class_distances(features, labels):
    ...
    class_means = np.zeros((num_classes, features.shape[1]))  # Fixed syntax error
...
ax1.set_title("UMAP Before Training (VGGNet)")
...
ax2.set_title("UMAP After Training (VGGNet)")
...
plt.savefig("VGGNet_umap_comparison_before_after_training_no_kronecker_jittered.png", bbox_inches='tight')
plt.show()
...
ax1.set_title("UMAP Before Training (VGGNet, Subsampled)")
ax1.set_xlabel("UMAP Component 1")
ax1.set_ylabel("UMAP Component 2")
...
ax2.set_title("UMAP After Training (VGGNet, Subsampled)")
ax2.set_xlabel("UMAP Component 1")
ax2.set_ylabel("UMAP Component 2")
...
plt.savefig("VGGNet_umap_comparison_before_after_training_no_kronecker_subsampled.png", bbox_inches='tight')
plt.show()
...
plt.title("Confusion Matrix on Test Set (VGGNet)")
plt.xlabel("Predicted Labels")
plt.ylabel("True Labels")
plt.savefig("confusion_matrix_top20_fusion_no_kronecker_VGGNet_UMP.png", bbox_inches='tight')
plt.show()