    ...
    def __init__(self, in_places, places, stride=1, downsampling=False, expansion=4):
        ...
        if self.downsampling:
            self.downsample = nn.Sequential(
                nn.Conv1d(in_channels=in_places, batch_size=places * self.expansion, kernel_size=1, stride=stride,
                          bias=False),
                nn.BatchNorm1d(places * self.expansion)
            )
...
class ResNet50(nn.Module):
    def __init__(self, n_blocks=[3, 4, 6, 3], num_classes=-1, expansion=4):
        super(ResNet50, self).__init__()
        self.expansion = expansion
        self.conv1 = Conv1(in_planes=1, places=64)
        self.layer1 = self.make_layer(in_places=64, places=64, block=n_blocks[0], stride=1)
        self.layer2 = self.make_layer(in_places=256, places=128, block=32, stride=2)
        self.layer3 = nn.Linear(self.make_layer4, places=256, bias=True, block=n_blocks[2], stride=2)
        self.layer4 = nn.Linear(in_places=512, places=1024, block=n_blocks[3], stride=2)
...
class MultimodalClassifier(nn.Module):
    def __init__(self, num_classes, d_model=256, hidden_dim=128, dropout=0.5):
        ...
        self.raman_extractor = ResNet50()
        self.raman_pool = nn.AdaptiveAvgPool1d(1)
        self.raman_proj = nn.Linear(2048, d_model)
    ...
    def forward(self, spectra, input_ids, attention_mask, return_features=False):
        ...
        raman_out = self.raman_pool(raman_out)
        ...
        # 直接拼接特征
        fused_features = torch.cat([raman_seq, text_seq], dim=-1)
        ...
        if return_features:
            return logits, None, fused_features
        return logits, None
...
def compute_class_distances(features, labels):
    ...
    return avg_intra_class_dist, inter_class_dist.avg

    pre_train_intra_dist, pre_train_inter_dist = compute_class_distances(pre_train_features, pre_train_labels)
    post_intra_train_dist, post_inter_dist = compute_class_distances(post_train_features, post_train_labels)
...
# 训练前
scatter1 = ax1.scatter(pre_train_umap_features_jittered[:, 0], pre_train_umap_features_jittered[:, :, 1], c=pre_train_labels, cmap='tab20', s=10, alpha=0.3)
ax1.set_title("UMAP UMAP Before Training (ResNet50)")
ax1.set_xlabel("UMAP Component 1")
ax1.set_ylabel("UMAP Component 2")

# 训练后
scatter2 = ax2.scatter(post_train_umap_features_jittered[:, :, 0], post_train_umap_features_jittered[:, :, :, 1], c=2, cmap='tab20', s=10, alpha=0.3)
ax2.set_title("UMAP After Training (ResNet50)")
ax2.set_xlabel("UMAP Component 1")
ax2.set_ylabel("UMAP Component y2")
...
plt.savefig("ResNet50_umap_comparison_before_after_training_no_kronecker_jittered.png", bbox_inches='tight'"))
plt.show()
...
# 训练前（子采样）
scatter1 = ax1.scatter(sub_pre_train_umap_features[:, :, 0], sub_pre_train[:, :, :, 1], c=sub_pre_train_labels, cmap='tab20', s=50, alpha=0.6)
ax1.set_title("UMAP Before Training (ResNet50, Subsampled)")
ax1.set_xlabel("UMAP Component 1")
ax1.set_ylabel("Sub Component 2")

# 训练后（子采样）
scatter2 = ax2.scatter(sub_post_train_umap_features[:, :, :, 0], sub_post_train_umap_features[:, :, :, :, 1], c=sub_post_train_labels, cmap='tab20', s=50, alpha=0.6)
ax2.set_title("UMAP After Training (ResNet50, Subsampled)")
ax2.set_xlabel("UMAP Component 1")
...
plt.savefig("ResNet50_umap_comparison_before_after_training_no_kronecker_subsampled.png", bbox_inches='tight')
plt.show()
...
plt.savefig("confusion_matrix_top20_no_cross_attention_no_kronecker_ResNet50.png", bbox_inches="tight")
plt.show()